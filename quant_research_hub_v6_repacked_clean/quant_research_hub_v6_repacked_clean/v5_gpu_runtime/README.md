...
这版直接把 GPU 主路线固定为 `xgboost_gpu`。

- 不再纠结 LightGBM GPU 验证
- `validate_only` 会新增 `xgboost_gpu_fit` 冒烟训练
- 候选实验生成优先 `xgboost_gpu`
- 建议先跑 `validate_only`，确认 `xgboost_gpu_fit=true`，再跑 `batch`
