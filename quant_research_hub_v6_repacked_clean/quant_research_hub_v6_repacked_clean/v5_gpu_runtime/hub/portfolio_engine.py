...
def build_latest_portfolio(latest_scores_df: pd.DataFrame, strategy: Dict[str, Any], out_dir: Path) -> Dict[str, Any]:
    ...
    cols = [c for c in ['date', 'code', 'ts_code', 'board', 'industry', 'close', 'pred_score', 'portfolio_weight', 'target_exposure', 'cash_buffer', 'amount_mean_20', 'vol_20', 'listed_days', 'in_hs300'] if c in df.columns]
