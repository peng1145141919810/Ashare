...
from hub.training_engine import train_and_predict
...
def execute_single_experiment_v5(
    ...
    try:
        ...
        if logger is not None:
            logger.info('训练与预测完成。n_features=%s elapsed=%.1fs', train_result['train_summary'].get('n_features', 0), time.time() - train_start)
        ...
        total_score = compute_total_score(train_result['train_summary'], portfolio_summary, config.get('evaluation_rules', {}))
        record.update({
        ...
        write_json(run_dir / 'run_summary.json', {'record': record, 'latest_portfolio': latest_portfolio_meta, 'portfolio_summary': portfolio_summary})
        if logger is not None:
            logger.info('候选实验完成。run_id=%s total_score=%.2f total_elapsed=%.1fs', run_id, float(record['total_score']), float(record['elapsed_seconds']))
        return {'run_id': run_id, 'record': record, 'run_dir': str(run_dir), 'elapsed_seconds': float(record['elapsed_seconds'])}
    except Exception as exc:
        record.update({'status': 'failed', 'total_score': -999.0, 'error_message': str(exc), 'elapsed_seconds': float(time.time() - exp_start)})
        append_record(registry_path, record)
        write_json(run_dir / 'run_summary.json', {'record': record, 'error': str(exc)})
        if logger is not None:
            logger.exception('候选实验失败。run_id=%s elapsed=%.1fs error=%s', run_id, float(record['elapsed_seconds']), str(exc))
        return {'run_id': run_id, 'record': record, 'run_dir': str(run_dir), 'elapsed_seconds': float(record['elapsed_seconds'])}
