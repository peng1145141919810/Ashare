...
def _load_runtime_capabilities(base_config: Dict[str, Any]) -> Dict[str, Any]:
    """读取 validate_only 产出的运行时能力摘要。"""
    hub_root = Path(str(base_config.get('hub_output_root', '')))
    val_path = hub_root / 'validation.json'
    payload: Dict[str, Any] = {'available_gpu_families': [], 'lightgbm_gpu_ready': False, 'xgboost_gpu_ready': False}
    try:
        if val_path.exists():
...
def _prune_model_families(route_space: Dict[str, Any], runtime_caps: Dict[str, Any]) -> Dict[str, Any]:
    """根据运行时能力裁剪模型家族。"""
    out = dict(route_space)
    families = list(out.get('model_families', []))
    if not runtime_caps.get('lightgbm_gpu_ready', False):
        families = [m for m in families if m != 'lightgbm_gpu']
    if not runtime_caps.get('xgboost_gpu_ready', False):
        families = [m for m in families if m != 'xgboost_gpu']
    if not families:
        families = ['lightgbm_auto', 'ridge_ranker', 'hist_gbdt']
...
def _seed_parents(base_config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    runtime_caps = _load_runtime_capabilities(base_config)
    gpu_families = list(runtime_caps.get('available_gpu_families', []))
    seed_families = ['lightgbm_auto'] + gpu_families + ['ridge_ranker']
    seed_families = list(dict.fromkeys(seed_families))
    seeds = []
    for feature_profile in ['baseline_plus', 'momentum_cross_section', 'vol_liq_quality']:
        for model_family in seed_families:
...
def _candidate_spec(route: str, parent: Dict[str, Any], space: Dict[str, Any], idx: int, cycle_index: int, hypotheses: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    if route == 'feature':
        ...
        # 模型路线优先让 GPU 家族得到预算。
        gpu_first = [m for m in mfs if m in {'lightgbm_gpu', 'xgboost_gpu'}] + [m for m in mfs if m not in {'lightgbm_gpu', 'xgboost_gpu'}]
        ...
        spec['feature_profile'] = 'generated_feature_pack'
        spec['model_family'] = ['lightgbm_gpu', 'xgboost_gpu', 'lightgbm_auto', 'ridge_ranker'][(idx + cycle_index) % 4]
    elif route == 'hybrid':
        spec['feature_profile'] = fps[(idx + cycle_index) % len(fps)]
...
def build_cycle_plan(base_config: Dict[str, Any], registry_df, cycle_index: int, diagnosis: Dict[str, Any], llm_client: LLMClient, cycle_dir: Path) -> Dict[str, Any]:
    ...
    runtime_caps = _load_runtime_capabilities(base_config)
    route_space = _prune_model_families(route_space, runtime_caps)
    ...
    gpu_profile['runtime_capabilities'] = runtime_caps
    ...
    for idx, route in enumerate(route_seq, start=1):
        ...
        # 若某 GPU 家族未通过 validate_only 冒烟测试，则直接改走 CPU 路线。
        if spec.get('model_family') == 'lightgbm_gpu' and not runtime_caps.get('lightgbm_gpu_ready', False):
            spec['model_family'] = 'lightgbm_auto'
        if spec.get('model_family') == 'xgboost_gpu' and not runtime_caps.get('xgboost_gpu_ready', False):
            spec['model_family'] = 'lightgbm_auto'
        sig = dict(spec)
