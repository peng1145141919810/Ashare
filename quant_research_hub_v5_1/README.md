...
本补丁做了三件事：
1. `validate_only` 不再只看 `import`，而是会真正做一次 **LightGBM GPU / XGBoost GPU 冒烟训练**。
2. 只有通过冒烟测试的 GPU 家族，才会进入下一轮候选实验；没通过的会自动从搜索空间移除。
3. 若 GPU 路线运行时仍失败，会把 `gpu_fallback_reason` 和 `gpu_runtime_caps` 写进 `train_summary.json`，方便追踪到底是 LightGBM GPU 构建问题，还是 XGBoost/CUDA 问题。
